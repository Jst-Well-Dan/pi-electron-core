...
/**
 * Project-scoped Feishu data root.
 *
 * PI_FEISHU_ROOT is passed to the detached daemon so it keeps using the
 * launcher project's config even if its working directory changes.
 */
export const ROOT_DIR = process.env.PI_FEISHU_ROOT?.trim()
  || join(process.cwd(), CONFIG_DIR_NAME, "feishu");
...
export const LOCKS_PATH = join(ROOT_DIR, "locks.json");
...
> = {
  domain: "feishu",
  groupPolicy: "open",
  // pi-electron-core runs Feishu through a local, project-scoped daemon.
  // Default to WebSocket so new projects do not try to bind 0.0.0.0:3001
  // for card callbacks, which commonly fails on Windows with EACCES.
  cardActionMode: "ws",
